...
/*   Updated: 2024/12/19 18:05:21 by aolabarr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	do_redirections(t_exec *node, int child, int num_procs, t_mini *shell)
{
	...
	fd_in = 0;
	fd_out = 0;
	dup_pipes(child, num_procs, shell);
	if (node->filename_in != NULL
		&& ft_strncmp(node->filename_in, HERE_DOC, ft_strlen(HERE_DOC)))
	{
		fd_in = open(node->filename_in, O_RDONLY);
		if (fd_in == ERROR)
			return (handle_error(ERR_OPEN), EXIT_FAILURE);
	}
	else if (node->filename_in != NULL
		&& !ft_strncmp(node->filename_in, HERE_DOC, ft_strlen(HERE_DOC)))
	{
		fd_in = create_here_doc_file(node->hdoc_content);
		if (fd_in == ERROR)
			return (EXIT_FAILURE);
	}
